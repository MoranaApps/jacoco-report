...
| `fail-on-threshold`             | Defines which thresholds must be met to pass the check. Accepts `true` (fail on any threshold), `false` (never fail), or a choice of **the** following values (in a list): `overall`, `changed-files-average`, `per-changed-file`. | No | true (fail on overall, changed-files-average, and per-changed-file) |
