...
| `skip-unchanged`                | If enabled (true), filters JaCoCo-related comments and violations to show only relevant changes.<br>It removes unchanged lines from tables, alerts from violations and skips entire comments for unmodified <br>modules and reports, reducing noise in PRs. | No                              | false                                                                                                 |
...
| `fail-on-threshold`             | Defines which thresholds must be met to pass the check. Accepts `true` (fail on any threshold), `false` (never fail), or a list: `overall`, `changed-files-average`, `per-changed-file`.                                                                    | No                              | true                                                                                                  |
...
- name: Publish JaCoCo Report
  ...
  with:
    ...
    fail-on-threshold: true  # Fail the GitHub action if defined threshold is not reached
        or
